...
namespace PharmacyManagmentSystem.Repositories
{
    public class InvoiceRepository : IInvoiceRepository
    {
        ...
        public async Task<IEnumerable<Invoice>> GetAllAsync()
        {
            ...
            return await _dbcontext.Invoices.Include(i => i.InvoiceItems).ThenInclude(ii => ii.Medicine).Include(i => i.User).ToListAsync();
        }
        public async Task<Invoice?> GetByIdAsync(int id)
        {
            // Load a single invoice + its items and the user 
            return await _dbcontext.Invoices.Include(i => i.InvoiceItems).ThenInclude(ii => ii.Medicine).Include(i => i.User).FirstOrDefaultAsync(i => i.Id == id);
        }
        ...
        public async Task<IEnumerable<Invoice>> GetByUserAsync(int id)
        {
        ...
        public async Task<IEnumerable<Invoice>> GetByDateRangeAsync(DateTime start, DateTime end)
        {
            ...
            return await _dbcontext.Invoices.Include(i => i.InvoiceItems).ThenInclude(ii => ii.Medicine).Where(i => i.DateCreated >= start && i.DateCreated <= end).ToListAsync();
        }
